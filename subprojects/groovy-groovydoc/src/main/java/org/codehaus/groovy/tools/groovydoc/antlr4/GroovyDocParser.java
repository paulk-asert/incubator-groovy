...
public class GroovyDocParser implements GroovyDocParserI {
    ...
    private Map<String, GroovyClassDoc> parseGroovy(String packagePath, String file, String src) throws RuntimeException {
        SourceUnit unit = SourceUnit.create("Groovydoc", src);
        Properties myProperties = new Properties(System.getProperties());
        myProperties.setProperty("groovy.attach.groovydoc", "true");
        CompilerConfiguration config = new CompilerConfiguration(myProperties);
